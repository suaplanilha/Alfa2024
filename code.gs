...
function doGet(e) {
  ...
  function logLoginHistory(email, username, loginDate, loginTime, status) {
    var historySheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('LoginHistory');
  ...
  function verifyOtp(email, otp) {
    ...
    for (var i = 1; i < values.length; i++) {
      var sheetEmail = values[i][1].toString().trim();
      var sheetOtp = values[i][8].toString().trim(); // OTP is stored in column I (index 8)
      ...
      if (sheetEmail === email.trim() && sheetOtp === otp.trim()) {
        ...
        // Clear the OTP from the sheet after successful verification
        sheet.getRange(i + 1, 9).setValue(''); // Clear the OTP in column I
  
        // Fetch sheet names from columns J to M
        var sheetsToCheck = [];
        for (var j = 9; j <= 12; j++) {
